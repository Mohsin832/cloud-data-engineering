...
INTERSECT SELECT city FROM sales.stores
...
with cte_overpricedProduct (category_id,
							product_name,
							list_price,
...
select category_id,
product_name,
list_price, AVG(list_price) OVER(PARTITION BY category_id) as  category_average_price
from production.products
GROUP BY category_id, product_name, list_price
)
select category_id, product_name, list_price, category_average_price 
from cte_overpricedProduct where list_price > category_average_price 
...
with cte_total_order_Count
AS (
	select staff_id , COUNT(*) as order_count
	from sales.orders group by staff_id
),	
cte_average as (
	select AVG(order_count) as average_count from cte_total_order_Count
)
SELECT oc.staff_id, ac.average_count FROM cte_total_order_Count oc
CROSS JOIN cte_average ac 
where oc.order_count > ac.average_count
...
AS ( 
	SELECT os.store_id, YEAR(os.order_date) as order_year , 
	SUM(oi.quantity * oi.list_price * (1 - oi.discount)) as total_revenue
	FROM sales.orders os
	INNER JOIN sales.order_items oi
	ON os.order_id = oi.order_id group by os.store_id, year(os.order_date)
)
...
CREATE TABLE sales.loyalty_cards (
 ...
 INSERT INTO sales.loyalty_cards VALUES (1003, 3,  0,    'Bronze', '2024-06-01');
 -- ERROR
 INSERT INTO sales.loyalty_cards VALUES (1001, 4,  100,  'Gold',   '2024-07-01');
 INSERT INTO sales.loyalty_cards VALUES (1004, 1,  -50,  'Silver', '2024-08-01');
 INSERT INTO sales.loyalty_cards VALUES (1005, 5,  200,  'Diamond','2024-09-01');


-- Q8.
-- The operations team realized that some orders in the database have
-- a shipped_date that is earlier than the orders_date, which is impossible.
...
CREATE TABLE test_orders (
     ...
     order_date    DATE NOT NULL,
     shipped_date  DATE, 
	 CONSTRAINT check_shipped_date CHECK(
